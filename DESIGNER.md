...
The Services navigation and pages extend the existing editorial system. Keep `SERVICES` between `OUR MISSION` and `UPCOMING EVENTS`; use the shared bilingual navigation data so desktop dropdowns, mobile accordions and the footer stay in sync. The Field Design & Development Services, Business Consultant, Marketing, Permit Approval, and Training pages should feel like open premium editorial/service pages: strong typographic hierarchy, generous whitespace, readable long-form copy, subtle section rhythm, and clear closing statements. Avoid tables, grids, identical rounded cards, or generic dashboard UI. English remains the default; Thai is selected through the existing language toggle and must retain valid UTF-8 glyphs.
