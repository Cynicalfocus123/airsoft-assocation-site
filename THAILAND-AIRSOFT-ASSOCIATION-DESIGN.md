...
Design system reviewed 2026-09-12. It covers the current 55-route bilingual release.
...
- The root-level TH / EN language service is available across every current and future page. English is the default, a deliberate choice persists locally through route navigation and refresh, and switching changes no route. Navigation, the actual footer, service pages, association pages, and the referee/member information pages consume shared bilingual data. Thai uses Noto Sans Thai, natural wrapping and real UTF-8 text; retain the English visual system and do not use locale-prefixed routes.
- Real routes only. Header and footer contain no hash links.
- The Contact Us page is an open editorial composition: oversized heading, body copy, a real `mailto:info@thaiairsoft.org` link, and thin divider rows for inquiry types. Do not add a form, card grid, or boxed contact options without approved requirements.
- The Upcoming Events calendar lists Force of Conquest followed by two non-clickable TBA rows. Keep Thailand National Airsoft 2026 out of this calendar while preserving its separate homepage featured hero.
